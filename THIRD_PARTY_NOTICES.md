...
本文件概述 MVM 0.3.0 运行时中直接使用或随应用分发的主要第三方组件。许可证名称来自当前锁定依赖的包元数据及随附许可证文件；它不是法律意见，也不取代各组件的完整许可证正文。

MVM-CPU/2 的加载器、整数解释引擎和有限 Darwin/Objective-C 桥为本项目自有实现，不包含 Darling 或 Wine 源码。测试应用由官方 Zig 0.15.2/Clang 编译器交叉编译；编译器不随安装包分发。样本的 .tbd 文件是本项目编写的 ABI 链接声明，不是 Apple SDK 或 Framework 二进制。桌面壳仍使用下列开源组件。
